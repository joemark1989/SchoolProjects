...
def position_check(selection, player, connect_four)
  ...
  while taken == true
    if row < 0
      check_selection = player_input
      row = 7
    elsif connect_four[row][check_selection] != "."
      row -= 1
    else
      connect_four[row][check_selection] = token
      taken = false
      check_winner = check_win(check_selection, token, connect_four, row)
    end
  end
  return check_winner
end
